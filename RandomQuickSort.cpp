...
int partition(double* array, int left, int right, int* quick_cmp) {
	...
	low = left;
	high = right + 1;
	pivotIndex = left + rand() % (right - left + 1);//pivot index를 위한 랜덤 데이터 생성
	pivot = array[pivotIndex]; //랜덤 데이터를 pivot으로 선택
	SWAP(array[left], array[pivotIndex], temp);//pivot과 left의 위치 교환
	pivotIndex = left;//pivot의 index를 바꿔줌
...
double* r_quick_sort(double* array, int left, int right, int*quick_cmp) {
	if (left < right) {//정렬할 범위가 2개 이상의 데이터일 경우
		int q = partition(array, left, right, quick_cmp);//q=pivot의 위치
		r_quick_sort(array, left, q - 1, quick_cmp);//pivot 기준 왼쪽 리스트 정렬
		r_quick_sort(array, q + 1, right, quick_cmp);//pivot 기준 오른쪽 리스트 정렬
	}
	return array;
}
...
int main() {
	...
	r_quick_sort(list1, left, right, &quick_cmp);
